...
import (
	...
	"github.com/i0li/proglog/internal/log"
...
type Config struct {
	ServerTLSConfig *tls.Config // 自身のローカルサーバを起動する際に使用される
	PeerTLSConfig   *tls.Config // 自身のローカルサーバまたは他のノードと接続する際に使用される
	DataDir         string
	BindAddr        string
	RPCPort         int
	NodeName        string
...
func (c Config) RPCAddr() (string, error) {
	// 「ホスト名:ポート番号」のように与えられた文字列からホスト名とポート番号を分割する
